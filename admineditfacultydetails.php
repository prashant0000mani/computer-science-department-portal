...
$qry="select userid from login_master where status='fclty';";
...
<div class="text-center" style="margin-top: 30px;"><label><a href="admineditstudentdetails.php"><u>Edit Student Details</u></a></label></div>
</div>
</div>
...
if($rows=mysqli_fetch_row($res))
{
    echo"
        ...
        <div class='well' style='background-color: aqua;'><h2 class='text-primary text-center' style='font-family:forte;'>Edit Faculty Details</h2></div>
        <center><form method='post' role='form' class='form-horizontal'>
        <div class='col-md-3'>
        <label class='control-label'>Department</label></div>
        <div class='col-md-7'>
        <input type='text' name='dept' value=$rows[1] class='form-control' /><br></div>
        <div class='col-md-3'>
        <label class='control-label'>Degree</label></div>
        <div class='col-md-7'>
        <input type='text' name='degree' value=$rows[2] class='form-control' /><br></div>
        <div class='col-md-3'> 
        <label class='control-label'>Join Year</label></div>
        <div class='col-md-7'>
        <input type='number' name='join' value=$rows[3] class='form-control' /><br></div>  
        <div class='col-md-6'> 
...
{   $user=$_COOKIE['user'];
    ...
    $join=$_POST['join'];
    $option=$_POST['choice'];
    $qry1="update faculty_master set department='$dept', degree='$degree', joinyear=$join where userid='$user' ;";
